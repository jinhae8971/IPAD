"""Seed catalog of agent categories and agents.

Mirror of ``src/data/agents.ts`` in the frontend. When the frontend is
wired to this backend, this becomes the source of truth.
"""
...
AGENTS: list[Agent] = [
    Agent(
        ...
        description=(
            "요구사항을 받아 모듈 경계, 데이터 모델, API 계약을 설계하고, "
            "단계적 구현 계획을 제시합니다."
        ),
        capabilities=[
            "요구사항 → 아키텍처 다이어그램 변환",
    ...
    Agent(
        ...
        description=(
            "주어진 사양과 코드베이스 컨벤션에 맞춰 파일을 생성·수정하고, "
            "테스트를 함께 작성합니다."
        ),
    ...
    Agent(
        ...
        description=(
            "변경 사항의 의도와 위험을 분리하여 평가하고, 잠재적 회귀와 "
            "보안 이슈를 표시합니다."
        ),
    ...
    Agent(
        ...
        description=(
            "뉴스 수집·요약·강세/약세/기술 토론을 거쳐 중장기 트레이딩 "
            "인사이트를 도출합니다."
        ),
    ...
    Agent(
        ...
        capabilities=["RSS/HTTP 어댑터", "중복 제거", "스키마 검증"],
        defaultModel="claude-haiku-4-5",
        tags=["ingest", "etl"],
    ),
    Agent(
...
def get_agents_by_category(category_id: str) -> list[Agent]:
    return [a for a in AGENTS if a.category == category_id]
